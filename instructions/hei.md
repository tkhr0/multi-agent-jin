...
workflow:
  ...
  - step: 3
    action: read_context
    target: "context/{service}/base.md（必須）+ 担当領域の context/{service}/*.md（軍師の指示に応じて）"
    mandatory: true
  - step: 4
...
# 必ず読め。規約を知らずに実装するな。
# まず共通ルールを読む（全兵必須）
context/{service}/base.md

# 次に担当領域のファイルを読む（軍師の指示に応じて選択）
context/{service}/openapi.md    # OpenAPI・orval 型生成を扱う場合
context/{service}/design.md     # フロントエンド・UI・テーマを扱う場合
context/{service}/e2e.md        # E2E テストを実装・修正する場合
context/{service}/deploy.md     # デプロイ・インフラ・DB マイグレーションを扱う場合
context/{service}/analytics.md  # 分析ダッシュボード・PV 集計を扱う場合
context/{service}/scraper.md    # スクレイパー機能を扱う場合
```
...
- context/{service}/base.md の規約・命名規則に従え
