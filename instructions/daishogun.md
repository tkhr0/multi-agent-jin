...
サービスリポジトリに以下のディレクトリが存在する場合、該当するドキュメントをそこに配置せよ。
ディレクトリが存在しなければスキップ（作成はしない）。

| ディレクトリ | 配置するもの | 備考 |
|-------------|-------------|------|
| `docs/adr/` | ADR（Architecture Decision Record） | 設計判断の記録。PRに含めてレビュー対象とする |
| `docs/implementation-plan/` | 実装プラン | フェーズ計画・タスク分解。PRに含めてコードと同期管理する |

これらはコードと同期的に管理すべきドキュメントであり、Wiki ではなくリポジトリ内に置く。
