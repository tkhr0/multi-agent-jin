...
|---------|------|------|
| 人間 | **王** | 最終意思決定者 |
| team-lead | **本陣** | 王の会話相手・司令塔・spawn 管理 |
| service-leader | **大将軍** | 技術管理・本陣の指示で軍師に任務振分 |
| feature-leader | **軍師** | 機能管理・兵への指示 |
| executor | **兵** | コーディング実働部隊 |

### feature ID の命名規則

名前衝突を防ぐため、feature ID には必ずプレフィックスをつける。

| 条件 | 形式 | 例 |
|------|------|-----|
| GitHub Issue あり | `{issue番号}-{feature名}` | `333-noindex`, `42-preview` |
| GitHub Issue なし | `{YYYYMMDD}-{feature名}` | `20260301-refactor-auth` |

この feature ID は以下の全てに統一して使う:

| 対象 | 例（issue #333 の場合） |
|------|------------------------|
| YAML ファイル名 | `projects/{service}/333-noindex.yaml` |
| ブランチ名 | `feature/333-noindex` |
| 軍師名 | `gunshi-333-noindex` |
| ログディレクトリ | `logs/{service}/333-noindex/` |
| YAML 内の `feature_id` | `333-noindex` |
...
王（人間）
  │
  ├─ 本陣-{service}（Claude session = team-lead）← 王の会話相手・司令塔
  │   ├─ 大将軍（subagent）← 本陣の指示で技術管理
...
- **本陣は王の意図を解釈して大将軍に指示を出す** → 実装レベルの技術判断は大将軍に委譲
...
### 本陣（team-lead）
- 王の会話相手（王の意図を理解し、要件を明確化する）
- 大将軍への指示出し（王の意図を実行可能な指示に変換）
- spawn 管理（大将軍・軍師からの spawn 要請を受けて実行）
- Memory MCP 管理（王の好み・ルール・重要な意思決定を記憶）
- **最小限の作業記憶のみ**（永続的な状態管理は大将軍の責任）
- 実装レベルの技術判断・タスク分解・ファイル編集は行わない

### 大将軍（service-leader）
- 本陣からの指示を受け、技術戦略を立案（サービス単位）
- `context/{service}.md` の守護者
- dashboard.md の更新責任者
- `projects/{service}/agents.yaml` の管理
- `{feature_id}.yaml` の新規作成
- GitHub Issues の作成・管理
- エスカレーション対応（軍師 → 大将軍 → 本陣 → 王）
...
| レイヤー | 生存期間 | 復元手段 |
|---------|---------|---------|
| 本陣 | サービスが存在する限り | Memory MCP + チームメンバー確認 |
| 大将軍 | サービスが存在する限り | `context/{service}.md` + YAML + GitHub Issues |
| 軍師 | 機能開始〜PRマージ | `projects/{service}/{feature_id}.yaml` |
...
大将軍 → 本陣   : SendMessage（報告・エスカレーション）
本陣 → 王       : 直接応答（team-lead として）
```
...
### 本陣の役割
本陣は王の意図を解釈して大将軍に指示を出す司令塔である。
大将軍は王と直接会話せず、本陣からの指示に従って技術管理を行う。
本陣は実装レベルの技術判断には立ち入らず、「何を・なぜ」を大将軍に伝える。
...
```
本陣    → Memory MCP（王の好み・ルール・重要な意思決定）
        + 作業記憶のみ（大将軍の agent_name、直近の指示概要）

大将軍  → GitHub Issues（進行中の作業の永続化）
        + context/{service}.md（技術文脈・規約・知見）
        + projects/{service}/*.yaml（各機能の状態・軍師の agent ID）

軍師    → projects/{service}/{feature_id}.yaml
...
| エージェント | 書くタイミング | 書く先 |
|------------|--------------|--------|
| 大将軍 | タスク分解依頼時 | `{feature_id}.yaml` 新規作成 |
| 大将軍 | 軍師 spawn 後 | `agents.yaml` に agent_id 追記 |
| 大将軍 | 軍師から報告受け取り時 | `{feature_id}.yaml` 更新 |
| 大将軍 | PR マージ後処理完了時 | `agents.yaml` から削除、`context/{service}.md` 更新 |
| 軍師 | タスク分解完了時 | `{feature_id}.yaml` 更新（タスク一覧） |
| 軍師 | 兵へ指示送信後 | `{feature_id}.yaml` 更新（hei_agent_id 記録） |
| 軍師 | 兵からの報告受け取り時 | `{feature_id}.yaml` 更新（task status） |
| 軍師 | PR 作成後 | `{feature_id}.yaml` 更新（pr_number 記録） |
...
├─ projects/
│   └─ {service}/
│       ├─ {feature_id}.yaml          # 機能ごとの状態・タスク分解（feature_id = "333-noindex" 等）
...
agents:
  ...
  gunshis:                       # 軍師一覧（機能単位）
    - feature_id: 42-preview
      agent_id: gunshi-42-preview
    - feature_id: 20260301-refactor-auth
      agent_id: gunshi-20260301-refactor-auth
```

### `projects/{service}/{feature_id}.yaml`
```yaml
feature_id: 42-preview
feature_name: プレビュー機能
service: myapp
github_issue: 42              # issue なしの場合は null
status: in_progress   # pending | in_progress | review | done
branch: feature/42-preview
gunshi_agent_id: gunshi-42-preview
tasks:
  - id: task_001
...
### `logs/{service}/{feature_id}/implementation_log.yaml`
...
- 機能ごとにブランチを切る（`feature/{feature_id}`、例: `feature/333-noindex`）
